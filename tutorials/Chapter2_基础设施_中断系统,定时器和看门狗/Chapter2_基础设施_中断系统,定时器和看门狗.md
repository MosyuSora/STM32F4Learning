...
- [**§1 中断和中断控制器 NVIC**](#1-中断和中断控制器nvic)
  - [1.1 STM32 的中断流程](#11-stm32的中断流程)（软件端向量表 → 硬件端 NVIC → 上下文保存 → 完整流程）
  - [1.2 硬件配置：两级使能](#12-硬件配置--两级使能)（EXTI 寄存器 → NVIC 使能与优先级分组）
  - [1.3 软件服务：IRQ Handler 注意事项](#13-软件服务--irq-handler的注意事项)（运行上下文 → 快进快出 → FPU 陷阱）
- [**§2 定时器 (TIM) 与时间管理**](#2-定时器-tim-与时间管理)
  - [2.1 硬件定时器分类](#21-硬件定时器和种类)（SysTick / 基本 / 通用 / 高级）
  - [2.2 时基单元：水桶模型](#22-计时器的核心灵魂时基单元)（PSC / CNT / ARR → 滴答时间计算）
  - [2.3 内核计时器 SysTick](#23-内核计时器systick)（HAL 接管 → 非阻塞延时）
  - [2.4 通用 TIM：瑞士军刀](#24-通用tim--瑞士军刀)（AF 绑定、PWM、输入捕获/输出比较、TIM 中断、定时器同步）
- [**§3 HAL 库实战（上篇）—— 中断按键彩色呼吸灯**](#3-第一个正式工程----中断按键彩色呼吸灯上篇)
  - [3.1 项目概述](#31-项目概述)（5 种运行模式）
  - [3.2 原理说明](#32-原理说明)（输出引脚 → 电容按键 → 待机/TIM6 → 唤醒按键）
  - [3.3 CubeMX 配置](#33-cubemx-配置)（引脚 → 定时器 → NVIC → 时钟树 → 生成代码）
  - [3.4 重点代码拆解](#34-重点代码拆解)（宏定义 → 状态变量 → 初始化 → 主循环 → 功能函数 → 回调）
  - [3.5 调试验证](#35-调试验证)（资源分析 → 断点步进 → 寄存器视图 → 变量观察/Logic Analyzer）
- [**§4 看门狗 (WDG) —— 系统的最后一道保险**](#4-看门狗-wdg--系统的最后一道保险)
  - [4.1 为什么需要看门狗](#41-为什么需要看门狗)（跑飞原因 → 倒计时炸弹 → 局限性）
  - [4.2 IWDG 独立看门狗](#42-iwdg独立看门狗)（LSI 独立时钟 → 寄存器与超时计算 → 喂狗位置设计）
  - [4.3 WWDG 窗口看门狗](#43-wwdg窗口看门狗)（窗口约束 → 防盲喂 → EWI 提前唤醒中断）
  - [4.4 IWDG vs WWDG 工程选型](#44-iwdg-vs-wwdg-工程选型)
  - [4.5 实战：给呼吸灯工程加上 IWDG](#45-实战给呼吸灯工程加上-iwdg)（CubeMX → 喂狗位置 → Stop 模式陷阱）
