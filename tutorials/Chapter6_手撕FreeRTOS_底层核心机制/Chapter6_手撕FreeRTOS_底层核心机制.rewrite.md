...
工头手里有几本**公共账**——谁在候场、谁在等钟点、传送带那头排了谁在等。平时只有工头和工人翻它。可急件闯进来也常要动这几本账（就像 §9 那样，往传送带上扔个信号、把某个工人叫回候场）。两边同时改同一本，账就会**改花**。这一节，讲的就是怎么护住这几本账。

### 11.1 账为什么会改花：改到一半，急件闯进来了

看一句最不起眼的 `count++`。它落到机器上其实是**三步**：把账上的数读进手里 → 加一 → 写回账上。假设工头刚把旧数 `5` 读进手里、还没写回，一封急件"哐"地闯进来，也把这个数从 `5` 改成 `6` 写回了账上；急件一走，工头接着把**自己手里**那个"`5`＋1＝`6`"写回去——**账页被改了两回，数却只涨了一个**。活生生丢了一笔。

裸机时代护它的办法很糙：改这几步之前，干脆把门铃线一掐（`__disable_irq` 关中断），改完再接回去。这一掐一接之间，谁也拍不了门，三步一气呵成——这就是**临界区**。RTOS 的临界区，是这招的精细版。
...
| 要护的账 | 用哪招 | 拦住谁 | 急件还闯得进吗 |
| --- | --- | --- | --- |
| 急件和工人都会碰 | `taskENTER_CRITICAL`（挂 BASEPRI 牌） | 红线内的急件 + 所有换班 | 红线外的照闯 |
| 只有工人会碰 | `vTaskSuspendAll`（喊一声别换班） | 只拦换班 | **全都照闯** |

牌子和喊话，护住了"大家怎么安全地翻同一本账"。可上面反复冒头那条**红线 `configMAX_SYSCALL_INTERRUPT_PRIORITY`**——它立的到底是什么规矩？说白了就一个问题：**急件闯进来时，到底能不能伸手去翻队列、信号量那几本账？** 下一节，专讲急件和 RTOS 怎么打交道。
