...
先看一个绕不开的现实难题。COMM 那路 UART 来了一帧数据，触发中断。可中断处理**必须极短**（§12 会讲为什么），绝不能在中断里慢慢解析协议、查表、组包——那会把整个系统卡住。可解析的活又不能不干。怎么办？
...
工头手里有几本**公共账**——谁在候场、谁在等钟点、传送带那头排了谁在等。这几本账，**会有两种人同时来翻**：一种是刚说的**急件**（中断闯进来也要动账，比如 §9 那样往传送带扔个信号）；另一种更隐蔽——**台上这个工人被换下去、顶上来的另一个工人**，也来动同一本账。两边同时改一本，账就**改花**。这一节，就讲怎么护住它。
...
所以有些代码，天生要求"**要么一步不做，要么一口气做完，中间绝不容别人插手**"。这样一段代码，就叫**临界区**（critical section）；**护临界区，就是把会来插手的人挡在门外，直到这几步跑完。**

那么，会来插手的到底是谁？就开头说的**两种人**：

- ① 一封**急件**（中断）闯进来，动了同一本账；
- ② **另一个工人**被换上台（任务切换），也来动这本账。

FreeRTOS 备了两把锁对付它们，**区别只在"挡谁"**：一把两种都挡，一把只挡第 ② 种。分开看。

### 11.2 第一把锁 taskENTER_CRITICAL：两种人一起挡

`taskENTER_CRITICAL()` 最彻底：挂锁期间**急件先候着、也不换班**，两种插手者一并挡住。奇的是——它明明只干了一件事，怎么两个都挡了？抄真码看（`portENTER_CRITICAL()` → `vPortEnterCritical()`，[port.c:475](../../reference/rtos_src/FreeRTOS-Kernel/portable/GCC/ARM_CM4F/port.c#L475)）：

```c
void vPortEnterCritical( void ) {
    portDISABLE_INTERRUPTS();       /* ← 唯一的实招：抬 BASEPRI，挡急件 */
    uxCriticalNesting++;            /*   记一下挂了几层 */
    if( uxCriticalNesting == 1 )
        configASSERT( ( portNVIC_INT_CTRL_REG & portVECTACTIVE_MASK ) == 0 );
}                                   /*   顺手断言：不许从急件(ISR)里进临界区 */

void vPortExitCritical( void ) {
    uxCriticalNesting--;
    if( uxCriticalNesting == 0 )    /* 减到 0 才真放开 */
        portENABLE_INTERRUPTS();
}
```

它唯一的实招，是第一句 `portDISABLE_INTERRUPTS()`——**抬 `BASEPRI`，把急件挡在门外**。那"抬 BASEPRI"具体是什么？它展开成 `vPortRaiseBASEPRI()`（[portmacro.h:213](../../reference/rtos_src/FreeRTOS-Kernel/portable/GCC/ARM_CM4F/portmacro.h#L213)），核心就一条汇编：

```asm
msr basepri, %0    @ %0 = configMAX_SYSCALL_INTERRUPT_PRIORITY
```

把那条红线的值写进 `BASEPRI`——CPU 从此把"加急等级不高于红线"的急件全挡门外（Cortex-M 里数值越大越不急，挡的是数值 ≥ 红线那批），比红线更急的照闯。这一条 `msr`，和 §6.4.2 PendSV 那两句本是同一条指令。

可它压根没写一句"禁止换班"，第 ② 种人——**换班**——怎么也一起停了？**关键就在这儿：换班不是自己发生的，它是被时钟中断（SysTick，一封"定时急件"，§7）驱动的。** 急件都被挡在门外，tick 进不来、`xTaskIncrementTick` 跑不了，也就没人被换上台。**掐住"中断"这个总闸，下游的"换班"就顺带停了——一招挡两人。** 这，正是"临界区连换班也一起挡住"的由来。

（另两点：`uxCriticalNesting` 让锁能**叠着挂**、减到 0 才真放开，防里层函数提前解锁；`configASSERT` 那句提醒你——**这个普通版临界区不许在急件里用**，急件要用 §12 的 `FromISR`。）用法两条铁律：临界区里**极短**、**绝不调用会阻塞的 API**（挂着锁还让工人去睡，全车间僵死）。

### 11.3 第二把锁 vTaskSuspendAll：只挡"另一个工人"，急件照放

第一把锁把急件也挡了，代价是连累一批中断的延迟。可很多时候，你要护的账**只有别的工人会碰、急件根本不碰**（比如工头从头数一遍花名册）。为这点事去拦急件，太亏。

于是有第二把、更轻的锁：**只挡第 ② 种人——换班，急件一概照放**。真身抄下来就一句：

```c
void vTaskSuspendAll( void ) {
    /* 没有临界区，一根门铃都不碰 */
    uxSchedulerSuspended = uxSchedulerSuspended + 1U;   /* 竖块牌：谁都别换班 */
}
```

把 `uxSchedulerSuspended` 自增一下，等于工头喊一嗓子"这会儿谁都别换班"。别的工人换不上来，**可急件照闯照办**——门铃根本没动。数完了 `xTaskResumeAll()` 把牌收回。（它敢**不加锁**自增，源码上头压着一段挺妙的并发论证，感兴趣可去啃那段注释。）

它**怎么做到"只挡换班、不挡急件"**？——正好和第一把相反：它不掐中断这个总闸，而是直接在"**换班**"那一步设了道关卡。§5.3 / §7 那两处伏笔就是这道关卡——`xTaskIncrementTick` 每 tick 进门先看这块牌：挂着，就**只把 tick 记账攒着**（pended ticks）、不真换人；`xTaskResumeAll` 摘牌时再一次性补上。于是中断照样一封封进来处理，只有"换人"这件事被摁住了。（代价同样是这段要短。）

### 11.4 到底用哪把锁：就看这本账会不会被急件碰

两把锁怎么选，一句话——**看你要护的这本账，会不会被急件碰**：

| 这本账…… | 用哪把锁 | 它挡住谁 | 急件还进得来吗 |
| --- | --- | --- | --- |
| **急件也会碰**（如 §12 中断里往队列扔数据） | `taskENTER_CRITICAL` | 急件 ＋ 换班（一并） | 红线内的进不来 |
| **只有工人会碰** | `vTaskSuspendAll` | 只挡换班 | **照进照办** |

到这儿，"什么是临界区、什么是调度器挂起"就不绕了：**两个都是"让一段代码不被别人插手地跑完"，区别只在挡谁——临界区掐"中断"这个总闸，连急件带换班一起停；调度器挂起只在"换班"那步设卡，急件照放，所以更轻。**
