...
void vTaskDelay( const TickType_t xTicksToDelay ) {
    if( xTicksToDelay > 0 ) {
        vTaskSuspendAll();                                  /* 挂号期间先喊"别换班"(§13.3) */
...
真正"挂号"的一步是 `prvAddCurrentTaskToDelayedList`：它算出**唤醒 tick = 当前 `xTickCount` + 要等的格数**，写进这人 `xStateListItem` 牌子上的那个号（`xItemValue`），再用 §3.3.3 的**有序插入**挂进等钟点区——于是**最早该醒的人永远排在队头**。而外面那对 `vTaskSuspendAll`/`xTaskResumeAll` 也别放过：挂号要动等钟点区这本公共账，所以先按 §13.3 的法子喊一嗓子"别换班"，挂稳了再放开。
...
信号量常被摆出一堆吓人的定义。可你只要记住一句就够了：**它就是 §8 那条传送带，把"货"撤掉、只留一个计数。** 传数据叫队列，传"有没有信号 / 还剩几个"就叫信号量。带上有名额＝有信号，空了＝没信号。就这么点区别。

它有两种用法，对应两种很实在的场景。

### 9.1 二值信号量：中断和任务之间的一根"叫醒绳"

先看一个绕不开的现实难题。COMM 那路 UART 来了一帧数据，触发中断。可中断处理**必须极短**（§14 会讲为什么），绝不能在中断里慢慢解析协议、查表、组包——那会把整个系统卡住。可解析的活又不能不干。怎么办？
...
**它只有两态**："有信号 / 没信号"（带上那唯一的名额，在或不在）。这一下，就把"中断要短"和"活要细"这对死结解开了：**中断只管发信号，重活全甩给任务。** 落到源码，它就是一条**长度 1、每件 0 字节**的队列——是不是和 §10 的锁几乎一模一样？没错，差别只在锁多记了个"主人"。

### 9.2 计数信号量：一叠令牌，管一池资源
...
| --- | --- | --- | --- |
| **队列**（§8） | 一笔笔数据 | `uxItemSize` 字节 | 环形缓冲 |
| **信号量**（§9） | 一个信号 / 一个计数 | **0** | 无（纯计数） |
| **互斥锁**（§10） | 一把唯一的钥匙 | 0 | **owner + 优先级继承** |
...
锁讲完了，任务协作还差最后一件。前面这三样——队列、信号量、互斥锁——你回头看 §9、§10 会发现它们**是同一副骨架变的**：都住在 `queue.c`，都是那条"传送带"（`Queue_t`）改改参数得来。可有一类协作，用队列硬套会别扭得很，FreeRTOS 干脆没让它做队列——那就是**事件组**。

## 11 事件组：一堵挂满信号灯的墙

### 11.1 有一种等待，队列表达不了：等"好几件事一起齐活"
...
### 11.3 回到 event_groups.c：一堵墙，就是一个整数 + 一张名单

#### 11.3.1 EventGroup_t：没有缓冲区，只有"灯排"和"名单"
...
对照着 §8 的 `Queue_t`（环形缓冲 `pcHead`/`pcWriteTo` + 两条等待链）你就看出**根本差别**了：
...
1. **条件先测、够了就不睡。** `prvTestWaitCondition` 一测，若此刻灯已经够了，直接返回，连阻塞都省了——这和 §8 队列"有货就不必等"是同一个精神。
...
3. 这条是**"无序"**事件链（`Unordered`），不像 §7 延时链按唤醒 tick 排序。因为灯的组合没有天然先后，点灯时只能**挨个扫一遍**名单去比对。

#### 11.3.3 点灯 xEventGroupSetBits：一次点灯，叫醒所有"够条件"的人
...
2. **每个人的"要求"就在他自己牌子上。** `listGET_LIST_ITEM_VALUE` 取出的，正是 §11.3.2 睡前打包进去的那个号；`& eventEVENT_BITS_CONTROL_BYTES` 把"AND 还是 OR、灭不灭"拆出来，剩下的就是他要的那几盏灯。
...
## 12 两种并发设计模式：从"机制"到"架构"

前面 §8–§11，我们把队列、信号量、互斥锁、事件组一件件拆到了源码——这些是**机制（mechanism）**，是砖。可写真实系统时，你脑子里想的往往不是"我该调 `xQueueSend` 还是 `xEventGroupSetBits`"，而是"这里是**谁产、谁消**""那里是**一处发生、多处关心**"。这种反复出现的结构，就是**设计模式（pattern）**，是图纸。这一节我们把话题从砖抬到图纸，挑两套并发编程里最经典的模式——它们都有几十年的学术渊源，也都能用手上这几块砖直接搭出来。

> 这是本章少有的"往上抽一层"的一节：不再拆新源码，而是把已拆过的原语，**放回它们在软件架构里的位置**。读源码是"向下钻"，认模式是"向上看"——两样都会，才算真懂并发。

### 12.1 生产者-消费者：一条有界缓冲，解耦快慢两端

第一套图纸，你其实在 §8 已经见过它的雏形。场景是这样：一头有人**源源不断地产**数据（串口收帧、传感器采样），另一头有人**慢慢地消费**（解析、落盘、上报）。两头速度对不齐——直接让产的人调用消费的人，就会**一快一慢互相拖**：产得快时消费方来不及、产得慢时消费方空等。
...
而 FreeRTOS 的漂亮之处在于：**队列（§8）把这套解法整个封装好了。** 你不用自己摆两个信号量加一把锁——`xQueueCreate` 给的队列，本身就是一个"自带同步的有界缓冲"：`xQueueSend` 在满时阻塞（相当于 `empty` 见 0）、`xQueueReceive` 在空时阻塞（相当于 `full` 见 0），内部的临界区（§13）替你做了那把互斥。所以在 FreeRTOS 里落地生产者-消费者，就是**建一个队列、两端各自收发**，几行而已。回头看 §9 会心一笑：**计数信号量**其实就是这套模式的退化版——货不带数据、只数个数，令牌即货。

一句话记住这套图纸的价值：**它把"谁产"和"谁消"在时间上解开了耦——两端不必同时在场、不必同速，缓冲负责削峰填谷。**

### 12.2 发布-订阅：一次发生，多处关心，而彼此互不相识
...
- **最轻的广播——事件组（§11）。** 一次 `xEventGroupSetBits`（发布）能同时唤醒多个等灯的任务（订阅者），这正是 pub-sub 的**一对多广播**内核。代价是它只传"发生了/没发生"这一个 bit，**不带数据载荷**。
- **要带数据的一对多——扇出（fan-out）。** 给每个订阅者各配一条队列（§8），发布者往每条队列各投一份副本。订阅者增减，只动"订阅表"，发布逻辑不变。
...
| FreeRTOS 落地 | 一条**队列**（§8） | **事件组**（§11）广播 / 每订阅者一条队列 |
...
这套"闯进来、办完、回原处"，第 2 章其实已经讲透了：门上得先装门铃（外设中断使能），门口的**保安队长 NVIC** 按急件的加急等级（优先级）决定放不放行、要不要打断当前的活（抢占），放行了就照**地址簿**（向量表）找到该办它的人（`IRQHandler`）。而急件闯进、办完退出那两下，**你手里的活计——那几个寄存器——硬件会自动先替你收进抽屉、办完再原样摆回来**（`R0-R3`/`R12`/`LR`/`PC`/`xPSR` 自动压栈、靠 `EXC_RETURN` 弹栈）。这套你不光第 2 章见过，§6 换班时又见了一遍——**PendSV 说白了就是一封"工头发给自己的急件"**。

所以急件**怎么进、怎么出**，我们不重复了。这一节补的，是 RTOS 真正新添的那点麻烦：**急件闯进来的时候，可能和台上的工人抢同一本账。**

工头手里有几本**公共账**——谁在候场、谁在等钟点、传送带那头排了谁在等。这几本账，**会有两拨人来插手改**：一拨是**门外闯进来的急件**（中断也要动账，比如 §9 那样往传送带扔个信号）；另一拨更隐蔽——**工头一听见墙上那口挂钟"当"地报点（§7 的 tick），就可能把台上的人换下、顶另一个上来**，这顶上来的工人也来翻同一本账。两拨同时改一本，账就**改花**。这一节就讲：工头改账那几步时，怎么把这两拨人都摁在外头。

### 13.1 什么叫"临界区"：一段"改到一半谁也别插手"的活
...
### 13.2 回到源码：两把锁的真身，一把一把拆

#### 13.2.1 头一招·闩门：门一闩，急件全挡在外
...
（另两点：`uxCriticalNesting` 让门能**上好几道闩**、退到 0 道才真开，免得里层函数替外层提前开了门；`configASSERT` 那句提醒——**这道普通门闩不许在急件里插**，急件另有专用通道 §14 的 `FromISR`。）用法两条铁律：门闩着的这几步要**极短**、且**绝不能在闩门时调用会让工人睡过去（阻塞）的 API**——门闩着人却去睡，全车间跟着僵死。

#### 13.2.2 一闩，为什么连"换班"也停了
...
### 13.3 闩门还是喊话：就看这本账会不会被急件碰

两招怎么选，一句话——**看你要护的这本账，会不会被急件碰**：

| 这本账…… | 使哪招 | 挡住谁 | 急件还进得来吗 |
| --- | --- | --- | --- |
| **急件也会翻**（如 §14 中断里往队列扔数据） | 闩门 `taskENTER_CRITICAL` | 急件 ＋ 换班（一并） | 红线内的撞不开门 |
...
## 14 中断里的 RTOS：急件怎么安全地喊人干活

§13 里，那条门框上的**红线**反复冒头，还立了句狠规矩——"红线外的急件绝不许碰账"。这条红线到底划的是什么、急件究竟怎么跟 RTOS 打交道，这一节讲透。而它正好是 §9.1 那根"叫醒绳"的**中断那一头**：当初说"中断只管拉绳、把重活甩给任务"，可这根绳在急件里到底怎么安全地拉——答案全在这儿。

### 14.1 门框上那条红线：谁准碰账，谁享"撞门"特权

第 2 章讲过，门口的保安队长 NVIC 会给每封急件盖一个**加急章**（优先级）。工头就着这个加急等级，在门框上画了条**红线**，把急件劈成两拨——这条红线，就是 `configMAX_SYSCALL_INTERRUPT_PRIORITY`：

- **红线以内**（不够急的那拨，Cortex-M 里数值 ≥ 红线）：**准进车间碰账**，但只能走"报备"通道（`xxxFromISR`，见下）。它们的另一个身份，正是 §13.2 那道**门闩拦得住**的急件——**正因为拦得住，工头闩着门改账时它们插不进来，账才护得住。**
- **红线以外**（比红线更急的，数值 < 红线）：享受**"闩门也拦不住、随到随进"的撞门特权**（给真·特大火警留的最低延迟），代价是——**绝不许碰工头任何一本账**。为什么？因为门闩拦不住它，一旦它去碰账，工头没有任何办法护住，账必乱。

Ch2 里那个"**数值越小越急**"的反直觉，到这儿才见真章。而且它牵出一条铁规矩：**那口挂钟（tick 中断）必须落在红线以内**——否则 §13.2 的"闩门连钟声一起挡"就不成立，工头闩着门也会被钟点戳穿、账就护不住了。（FreeRTOS 源码注释里专门红着脸警告过这条。）

### 14.2 急件不能"等"：所以有一整套 FromISR
...
到这儿，PART3 的中断线就通了：急件闯门、护账的闩门与喊话（§13）、红线 + FromISR + 退出即换班（§14）。任务怎么活、怎么协作、急件怎么安全掺和进来——三条线齐了。可这一路上，任务栈、TCB、队列缓冲、信号量、锁，桩桩都在吃 RAM——**它们到底从哪块地里长出来的？** 下一 PART，就去见这块地的两位管家：**栈**和**堆**。
...
### 15.1 一条内存隔离的谱系：8 位机 → MMU → Cortex-M
...
### 15.2 双栈 MSP / PSP：没有 MMU，也能把现场隔开
...
MSP 那一半，是留给内核和中断的**公用栈**：急件闯进来（§13）时 CPU 切到 handler 模式、用 MSP，**不去啃任何一个任务本就不宽裕的栈**；芯片一上电、`main` 还没交权前（§6.1），用的也是 MSP。
...
4. **干活时被急件打断（§13 / §14）**：硬件把一套异常帧（`R0-R3/R12/LR/PC/xPSR`）压进**当前这块 PSP 栈**（SP 又下探一截），再切 MSP 去跑 handler；handler 返回时从 PSP 弹回，任务接着跑。**——所以任务栈必须替随时可能来的急件预留出这份空间。**
5. **被换下台（§6.4）**：PendSV 把 `R4-R11` 也补压进它的 PSP 栈，把此刻栈顶存回 `pxTopOfStack`；下次上台，再从 TCB 取回栈顶、反着弹出来。

这里要把 §15.2 那两根栈指针，落到**一块真实的 RAM**上看清楚——初学者最容易糊涂的，就是把 MSP、PSP 想成"两块内存"。**它俩不是两块内存，是同一块 RAM 里两段地址上的两根指针。** 看下图这块 SRAM：
...
- **中间从堆 `ucHeap` 里挖出的一段段，是每个任务各自的栈，`PSP` 指着"当前在跑"的那个**（图里是任务 A）。**换任务，本质就是把 PSP 从 A 那段改指 B 那段**——这正是 §6 PendSV 收尾那句 `msr psp, r0` 干的事。
...
- 最要紧的一句：**这几段是同一块 RAM，段与段之间没有任何硬件墙。** 任务栈一旦写过界，照样踩烂隔壁（邻居的栈、TCB）——这就是下一节栈溢出的由来。想要真正的"墙"，得另配 MPU（§15.2 末尾那句），那是再加一层的事，不是这里多任务的地基。

收束一句：**任务的栈，就是它"能被暂停、能恢复"这件事的物理载体；SP 每一次升降，都是这条执行流在自己那块地上进退。** §1 说任务是"能暂停再继续的执行流"，到这儿才算落到了硬件上。

### 15.4 用好它：栈开多深、怎么知道快爆了
...
## 16 heap_4：动态对象的 RAM 管家

§15 的栈，是每个任务创建时就分好的固定 RAM。可 `xQueueCreate`、`xSemaphoreCreateMutex` 这些**运行中现造**的对象，内存得当场**动态申请**——这就是 `pvPortMalloc` / `vPortFree`，扮演的正是 C 里 `malloc` / `free` 的角色，只不过 FreeRTOS 没用 libc 的，而是自己实现了一套（还不止一套）。
...
### 16.3 回到 heap_4.c：一块地怎么分、怎么还、怎么合

#### 16.3.1 每块地钉一块牌：BlockLink_t，最高位当"已占"旗
...
#### 16.3.2 分地 pvPortMalloc：首次适配，富余就切一块
...
#### 16.3.3 还地 + 合并 prvInsertBlockIntoFreeList：抗碎片的命根子
...
还回来一块地，它左看看、右看看：**前邻居若紧挨着，两块并一块；后邻居若也紧挨着，再吞一块。** 于是零碎的相邻空闲地会**自动长回大块**——这，正是 heap_4 比"能还却不合并"的 heap_2 强的地方，也是 §16.2 说它"抗碎片"的全部底气。
...
### 16.4 剩多少、最惨剩过多少：两个水位

地主还留着两个数：`xFreeBytesRemaining`（当前总共还剩多少空闲字节）、`xMinimumEverFreeBytesRemaining`（**历史上最少的时候剩过多少**——和 §15 栈的高水位一个思路）。`xPortGetFreeHeapSize()` / `xPortGetMinimumEverFreeHeapSize()` 读的就是它俩。
...
对象从哪来——栈（§15）和堆（§16），到此都清楚了。那么把整条线收个尾：一个任务从被创建、上台、等待、协作、到最后被删，这一整趟"活法"，该合到一张图上看看了。这就是下一 PART 的**全景**。
...
### 17.3 阻塞 vs 挂起：都是"下台"，差在有没有"回来的条件"
...
### 17.4 删除与收尸：为什么一个任务不能痛快地删掉自己

`vTaskDelete` 最反直觉的一点是：**任务往往不能当场把自己删干净。** 想想就明白了——一个任务正站在**它自己的那块栈**上运行（§15），你让它此刻就把这块栈和自己的 TCB 用 `vPortFree` 还给堆？那它脚下的地就被抽了，函数都返回不回去。**人不能一边站在木板上，一边锯断这块木板。**
...
于是整条链串起来了：`vTaskDelete` 判死刑、挂进终止链、记一笔 → 任务被换下台后不再运行 → 空闲任务某次轮到它跑，翻出终止链、把栈和 TCB 还给 heap_4（§16）。**这就是为什么删一个任务后，堆空间不是"立刻"回来，而是"等空闲任务跑一趟"才回来**——你若删完立刻 `xPortGetFreeHeapSize()`，很可能一点没涨，得等 CPU 空下来让 Idle 收完尸才涨。

> 补一句对称的：如果你删的是**别的**任务（不是自己），那任务没站在自己栈上，`vTaskDelete` 就地 `prvDeleteTCB` 直接释放，用不着劳烦空闲任务。**只有"自杀"才需要收尸人**——因为没人能亲手拆掉自己脚下的地。

### 17.5 把一生连起来读

现在回看 §17.2 那张图，它其实就是本章前十六节的一条总索引：
...
> （待写）一条项目日志把 §1–§16 的机制串成一条链，纯机制串讲，无排查。
