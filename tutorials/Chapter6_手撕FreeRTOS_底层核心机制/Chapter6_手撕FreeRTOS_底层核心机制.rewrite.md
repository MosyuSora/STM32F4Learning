...
| 这本账…… | 使哪招 | 挡住谁 | 急件还进得来吗 |
| --- | --- | --- | --- |
| **急件也会翻**（如 §12 中断里往队列扔数据） | 闩门 `taskENTER_CRITICAL` | 急件 ＋ 换班（一并） | 红线内的撞不开门 |
...
这里要把 §13.2 那两根栈指针，落到**一块真实的 RAM**上看清楚——初学者最容易糊涂的，就是把 MSP、PSP 想成"两块内存"。**它俩不是两块内存，是同一块 RAM 里两段地址上的两根指针。** 看下图这块 SRAM：
...
- 最要紧的一句：**这几段是同一块 RAM，段与段之间没有任何硬件墙。** 任务栈一旦写过界，照样踩烂隔壁（邻居的栈、TCB）——这就是下一节栈溢出的由来。想要真正的"墙"，得另配 MPU（§13.2 末尾那句），那是再加一层的事，不是这里多任务的地基。
