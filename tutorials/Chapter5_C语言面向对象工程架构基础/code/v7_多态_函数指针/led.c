...
int led_init(Led_t *led, uint8_t pin, void (*on)(Led_t*), void (*off)(Led_t*), void (*toggle)(Led_t*)){led->pin=pin;led->is_on=false;led->do_on=on;led->do_off=off;led->do_toggle=toggle;printf("  [Led] Pin%d init polymorphic\n",pin);return 0;}
int led_on(Led_t *led){led->do_on(led);return 0;}
int led_off(Led_t *led){led->do_off(led);return 0;}
int led_toggle(Led_t *led){led->do_toggle(led);return 0;}
int pwm_led_init(PwmLed_t *pwm, uint8_t pin, uint8_t duty){led_init(&pwm->base,pin,pwm_on,pwm_off,pwm_toggle);pwm->duty=duty;return 0;}